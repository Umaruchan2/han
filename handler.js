            ...
            if (typeof plugin.all === 'function') {
                try {
                    ...
                    for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                        ...
                        if (data.exists) m.reply(`*🗂️Plugin:* ${name}\n*👤Sender:* wa.me/${m.sender.split`@`[0]}\n*💬Chat:* ${m.chat}\n*💻Command:* ${m.text}\n\n\`\`\`${format(e)}\`\`\``.trim(), data.jid)
                    }
                }
            }
                ...
                try {
                    ...
                    if (e) {
                        ...
                        if (e.name) {
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists) m.reply(`*🗂️Plugin:* ${m.plugin}\n*👤Sender:* wa.me/${m.sender.split`@`[0]}\n*💬Chat:* ${m.chat}\n*💻Command:* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\``.trim(), data.jid)
                            }
                        }
                        m.reply(String(e))
                    }
...
export async function participantsUpdate({ id, participants, action }) {
 ...
 let wel = await new Canvas.Welcome()
  ...
  var buffa = await wel.toBuffer()
                       ...
                       this.sendHydrated(id, text, wm + '\n\n' + botdate, action === 'add' ? wel.toBuffer() : lea.toBuffer(), gcwangsaf, (action == 'add' ? 'Sakura Group' : 'Nitip Gorengan'), user.split`@`[0], 'Telpon', [
      ['🎀 Menu', '/menu'],
      ['🪄 Test', '/ping'],
      ['intro','.intro'],
